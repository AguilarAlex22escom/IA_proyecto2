...
def create_nearest_neighbor_model(split, moment):
    print("Working with One Nearest Neighbor model...")
    ...
    if split == "hold_out": 
        ...
        model = neigh.fit(x_train_scaled, y_train_hd)
        y_pred = model.predict(x_test_scaled)
        accuracy = accuracy_score(y_test, y_pred)
        matrix = confusion_matrix(y_test, y_pred)
        
        y_scores = neigh.predict_proba(x_test_scaled)
        ...
        return {"Model" : "Nearest neighbor", "Split method": "Hold out", "Accuracy" : accuracy.round(4), "Moments" : moment}
        ...
        for train_index, test_index in skf.split(x_train, y_train):
            ...
            model = neigh.fit(x_train_scaled, y_train_skf)
            y_pred = model.predict(x_test_scaled)
        ...
        return {"Model" : "Nearest neighbor", "Split method": "5-Fold cross", "Accuracy" : average_accuracy.round(4), "Moments" : moment}
        ...
        average_area = np.mean(areas)
        make_printers(average_accuracy, split, moment, average_matrix, average_area)
        return {"Model" : "Nearest neighbor", "Split method": "10-Fold cross", "Accuracy" : average_accuracy.round(4), "Moments" : moment}
    else:
