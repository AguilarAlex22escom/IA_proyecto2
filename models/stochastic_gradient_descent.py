...
def create_stochastic_gradient_descent_model(split, moment):
    ...
    sgd = SGDClassifier(loss="log_loss", penalty="l2", max_iter=100)
    ...
    if split == "hold_out": 
        ...
        model = sgd.fit(x_train_scaled, y_train_hd)
        y_pred = model.predict(x_test_scaled)
        accuracy = accuracy_score(y_test, y_pred)
        matrix = confusion_matrix(y_test, y_pred)

        y_scores = sgd.predict_proba(x_test_scaled)
        ...
        return {"Model" : "Stochastic gradient descent", "Split method": "Hold out", "Accuracy" : accuracy.round(4), "Moments" : moment}
        ...
        for train_index, test_index in skf.split(x_train, y_train):
            ...
            model = sgd.fit(x_train_scaled, y_train_skf)
            y_pred = model.predict(x_test_scaled)
        ...
        return {"Model" : "Stochastic gradient descent", "Split method": "5-Fold cross", "Accuracy" : average_accuracy.round(4), "Moments" : moment}
        ...
        average_area = np.mean(areas).round(3)
        make_printers(average_accuracy, split, moment, average_matrix, average_area)
        return {"Model" : "Stochastic gradient descent", "Split method": "10-Fold cross", "Accuracy" : average_accuracy.round(4), "Moments" : moment}
    else:
