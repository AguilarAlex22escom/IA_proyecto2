...
def create_gaussian_naive_bayes_model(split, moment):
    print("Working with Naive Bayes model (Gaussian algorithm)...")
    ...
    if split == "hold_out": 
        ...
        model = gnb.fit(x_train_scaled, y_train_hd)
        ...
        return {"Model" : "Naive Bayes (Gaussian algorithm)", "Split method": "Hold out", "Accuracy" : accuracy.round(4), "Moments" : moment}
        ...
        for train_index, test_index in skf.split(x_train, y_train):
            ...
            model = gnb.fit(x_train_scaled, y_train_skf)
            y_pred = model.predict(x_test_scaled)
        ...
        return {"Model" : "Naive Bayes (Gaussian algorithm)", "Split method" : "5-Fold cross", "Accuracy" : average_accuracy.round(4), "Moments" : moment}
        ...
        average_area = np.mean(areas)
        make_printers(average_accuracy, split, moment, average_matrix, average_area)
        return {"Model" : "Naive Bayes (Gaussian algorithm)", "Split method" : "10-Fold cross", "Accuracy" : average_accuracy.round(4), "Moments" : moment}
    else:
